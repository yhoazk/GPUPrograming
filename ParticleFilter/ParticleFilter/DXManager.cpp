...
bool CDXManager::Initialize(IDXGIAdapter* pAdapter, bool bUseCPU,HWND hWnd)
{
	...
	dscd.BufferDesc.Width = 1024;
	dscd.BufferDesc.Height = 1024;
	...
	if (bUseCPU)
	{
		HRESULT hr = (!hWnd) ? D3D11CreateDevice(
			...
			D3D11CreateDeviceAndSwapChain(
				...
				&dscd, &m_pSwapChain, &m_pDevice, &Detected,
				&m_pContext);
		if (FAILED(hr))
			return false;
	}
	...
	{
		HRESULT hr = (!hWnd) ? D3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN,
			NULL, 0,
			&Requested, 1, D3D11_SDK_VERSION, &m_pDevice, &Detected,
			&m_pContext) :
	...
	{
		HRESULT hr = (!hWnd)?D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE,
			NULL, 0,
			&Requested, 1, D3D11_SDK_VERSION, &m_pDevice, &Detected,
			&m_pContext):
			D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE,
				NULL, 0,
				&Requested, 1, D3D11_SDK_VERSION,&dscd,&m_pSwapChain,&m_pDevice, &Detected,
				&m_pContext);
		if (FAILED(hr)) return false;
	}
	//DXGICreateSwapChain
...
ID3D11Buffer* CDXManager::LoadBuffer(
	void* pInData, unsigned long ulElementSize,
	unsigned long ulElements)
{
    ...
    /*                         GPU            CPU
	D3D11_USAGE_DEFAULT        W/R            n/a
	D3D11_USAGE_IMMUTABLE       R             wr once
	D3D11_USAGE_DYNAMIC         R              W
	D3D11_USAGE_STAGING        n/a            W/R
	*/
	...
	m_pDevice->CreateBuffer(&dbd, pInData?&dsd:0, &pNewBuffer);
	return pNewBuffer;
}
void CDXManager::StoreBuffer(ID3D11Buffer* pISB,
	...
	void* pOutData)
{
	...
	D3D11_BUFFER_DESC dbd;
	dbd.BindFlags = 0;
	dbd.ByteWidth = ulElementSize*ulElements;
...
void CDXManager::UpdateConstantBuffer(
	ID3D11Buffer* pCB, void* pSource, unsigned long ulBytes)
{
	D3D11_MAPPED_SUBRESOURCE ms;
