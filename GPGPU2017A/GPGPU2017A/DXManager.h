...
protected:
    ...
    IDXGISwapChain* m_pSwapChain; // cadena de intercambio (front and back buffers, visible en pantalla)
...
public:
    ...
    IDXGISwapChain* GetSwapChain() { return m_pSwapChain; } // accesor
    /*Load from CPU to GPU*/
    ID3D11Buffer* LoadBuffer(void* pData, unsigned long ulElementSize, unsigned long ulElements);
    /*From GPU to CPU*/
    void StoreBuffer(ID3D11Buffer* pISB, unsigned long ulElemenSize, unsigned long ulElements, void* pOutData); //isb interface structured buffer
    ...
    ID3D11Texture2D* CDXManager::LoadTexture(
        ...
        float(*pAlphaFunction)(float r, float g, float b));
